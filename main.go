...
func main() {
	...
	defaultAWSCfg, err := config.LoadDefaultConfig(context.TODO(), config.WithRegion(s3Region))
	if err != nil {
		log.Fatal("Couldn't load default AWS config")
	}
	awsClient := s3.NewFromConfig(defaultAWSCfg)
